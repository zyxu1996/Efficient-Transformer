...
pretrained_save_dir = './pretrained_weights'
...
def resnet18_v1b(pretrained=False, **kwargs):
    ...
    if pretrained:
        old_dict = model_zoo.load_url(model_urls['resnet18'], model_dir=pretrained_save_dir)
...
def resnet34_v1b(pretrained=False, **kwargs):
    ...
    if pretrained:
        old_dict = model_zoo.load_url(model_urls['resnet34'], model_dir=pretrained_save_dir)
        model_dict = model.state_dict()
        old_dict = {k: v for k, v in old_dict.items() if (k in model_dict)}
        model_dict.update(old_dict)
        model.load_state_dict(model_dict)
...
def resnet50_v1b(pretrained=False, **kwargs):
    ...
    if pretrained:
        print('load pretrain resnet50_v1b')
        old_dict = model_zoo.load_url(model_urls['resnet50'], model_dir=pretrained_save_dir)
        model_dict = model.state_dict()
        old_dict = {k: v for k, v in old_dict.items() if (k in model_dict)}
        model_dict.update(old_dict)
        model.load_state_dict(model_dict)
...
def resnet101_v1b(pretrained=False, **kwargs):
    ...
    if pretrained:
        old_dict = model_zoo.load_url(model_urls['resnet101'], model_dir=pretrained_save_dir)
        model_dict = model.state_dict()
        old_dict = {k: v for k, v in old_dict.items() if (k in model_dict)}
        model_dict.update(old_dict)
        model.load_state_dict(model_dict)
...
def resnet152_v1b(pretrained=False, **kwargs):
    ...
    if pretrained:
        old_dict = model_zoo.load_url(model_urls['resnet152'], model_dir=pretrained_save_dir)
        model_dict = model.state_dict()
        old_dict = {k: v for k, v in old_dict.items() if (k in model_dict)}
        model_dict.update(old_dict)
        model.load_state_dict(model_dict)
...
def resnet50_v1s(pretrained=False, root=pretrained_save_dir, **kwargs):
...
def resnet101_v1s(pretrained=False, root=pretrained_save_dir, **kwargs):
...
def resnet152_v1s(pretrained=False, root=pretrained_save_dir, **kwargs):
